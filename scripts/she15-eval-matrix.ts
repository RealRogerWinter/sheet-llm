 ...
 * cheapest-first Groq models), routing the WHOLE orchestrator to each via
 ...
 *
 * Requires ANTHROPIC_API_KEY (baseline) and GROQ_API_KEY (Groq rows) in env.
...
// Cheapest-first (SHE-15 locked decision). gpt-oss-120b / llama-3.3-70b are
// available as rungs-up if the trio fails (add to --models explicitly).
const ALL_CANDIDATES: Candidate[] = [
  { label: 'baseline', env: {}, estPerCaseUsd: 0.05, isBaseline: true },
  { label: 'groq:llama-3.1-8b-instant', env: GROQ_ENV('llama-3.1-8b-instant'), estPerCaseUsd: 0.015 },
  { label: 'groq:openai/gpt-oss-20b', env: GROQ_ENV('openai/gpt-oss-20b'), estPerCaseUsd: 0.015 },
  { label: 'groq:qwen/qwen3-32b', env: GROQ_ENV('qwen/qwen3-32b'), estPerCaseUsd: 0.015 },
...
function main(): number {
  ...
  const needsAnthropic = selected.some((c) => c.isBaseline)
  const needsGroq = selected.some((c) => !c.isBaseline)
