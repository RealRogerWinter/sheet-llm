...
| `SL_HAIKU_SINGLE_CALL` | off | bool | **SHE-19 PR2** free-tier single-call collapse for EDITS. `1` → an edit on the `free` tier (with an `editedScore`) runs as ONE Haiku `tool_choice:'auto'` call (`orchestrator/haikuSingleCall.ts:runHaikuSingleCall`) that picks the action AND emits the ops, replacing the 2-call dispatcher→handler path; the result still flows through `finalizeDispatchResult` (preservation check + replacement gate apply). **Off by default, hosted-free-tier-only** (daily-quota / training-capture precedent — self-hosted/local never get it); Anthropic-only (else `MultiToolUnsupportedError`); ANY throw falls back to the 2-call path (turn never dropped) — INCLUDING a structural edit that changed nothing (empty ops / echoed bars, e.g. a "make it Dorian" transform Haiku returns unchanged), which throws `HaikuSingleCallError` rather than silently committing the original score. If the edit STILL changes nothing after the fallback, `finalizeDispatchResult` surfaces a "couldn't apply that change" notice instead of re-rendering the same score. `flags.ts:isHaikuSingleCallEnabled` |
