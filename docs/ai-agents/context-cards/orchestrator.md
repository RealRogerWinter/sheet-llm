...
- `flags.ts` — env accessors, read fresh every call; `isSectionalGenEnabled()` reads `SL_SECTIONAL_GEN` (default ON); `isBoundedGenEnabled()` reads `SL_BOUNDED_GEN` (default ON); `isStreamAbortEnabled()` reads `SL_STREAM_ABORT` (default OFF); `isHaikuSingleCallEnabled()` reads `SL_HAIKU_SINGLE_CALL` (default OFF).
- `haikuSingleCall.ts` — **SHE-19 PR2** free-tier single-call collapse. `runHaikuSingleCall()` = ONE Haiku `tool_choice:'auto'` call (the only real multi-tool `auto` in the system) exposing the 5 emit-tools; picks the action AND emits ops, replacing the 2-call dispatcher→handler path. Gated in `index.ts` by `editedScore && getGenerationTier()==='free' && isHaikuSingleCallEnabled()`, routed through `finalizeDispatchResult` (preservation + replacement gate apply); Anthropic-only (`AnthropicProvider.multiToolCall`, else `MultiToolUnsupportedError`); ANY throw → fall back to the 2-call path (turn never dropped). A structural edit that produced NO change (empty ops / echoed bars — the "make it Dorian" regression) throws `HaikuSingleCallError` (via `assertEditChangedScore` → `isNoOpEdit`) so it falls back instead of silently committing the same score; a still-no-op edit after fallback gets the `noticeNoOpEdit` "couldn't apply" message in `finalizeDispatchResult`. Off by default, hosted-free-tier-only.
